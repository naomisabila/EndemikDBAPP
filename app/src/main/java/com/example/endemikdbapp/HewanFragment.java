...
public class HewanFragment extends Fragment {
    ...
    private List<Endemik> allHewanData = new ArrayList<>();
        ...
        RetrofitClient.getInstance().getEndemikList().enqueue(new Callback<List<Endemik>>() {
            ...
            public void onResponse(@NonNull Call<List<Endemik>> call, @NonNull Response<List<Endemik>> response) {
                ...
                if (response.isSuccessful() && response.body() != null) {
                    allHewanData = new ArrayList<>();
                    for (Endemik item : response.body()) {
                        if ("Hewan".equals(item.getKategori())) {
                            allHewanData.add(item);
                        }
                    }
                    applyFilter();
                }
    ...
    private void applyFilter() {
        ...
        if ("Semua".equals(currentRegion)) {
            filteredList.addAll(allHewanData);
        } else {
            for (Endemik item : allHewanData) {
